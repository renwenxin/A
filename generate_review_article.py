...
# ------------------------------------------------------------------
# 模板示例（用户最新手写复盘 2026-08-14，盘中视角 — 招牌技法：量价先行/板块强弱对照/
# 缠论视角技术推演(分型/箱体或中枢雏形/背离风险/缺口双读/级别条件)/核心矛盾/
# 结构分化/快讯驱动基本面/预测验证与反证/个人仓位纪律/哲学收束。
...
- 全程第一人称（"我认为/在我看来/我判断"），像老朋友复盘盘面，敢下判断，但重结论必须给证据链，不机械播报数据。
- 量价先行：开篇先摆今日成交额与"较前一日的放/缩量"，随即给量价联动的双分支推演（"若能温和放量，则……；反之，若量能再度萎缩，则……"），量是第一判断，别把成交额埋进数据堆里只说个数。
- 把指数当人写，用生活化比喻（如"大盘做体检""回血""抽身半秒""留一分清醒"这类），允许适度拟人，别干巴巴只贴点位。
- 技术面按缠论视角写：先说结构位置，再说量能配合，再说当下级别判断，最后落动作。可写分型、箱体/中枢雏形、缺口回踩确认、背离风险、推动浪后的正常回撤、级别共振；数据里没有的5分钟MACD、精确二买三买、中枢边界不要硬编。
- 技术面要讲具体结构并推演后果：站上/跌破均线、阳包阴反包、推动浪、顶背离、缺口回补与性质、整数关口、年线争夺，配量能解读，再落一句"守住会怎样、失守该怎么做"的条件结论（如"若放量突破并企稳3975之上则转强；若缩量反抽遇阻则抛压仍重"）。
- 基本面只按获取到的快讯来：有快讯就写"快讯触发→板块异动→龙头/跟风结构→持续性判断"；没有相关快讯就只写盘面资金流，不编政策、业绩、涨价、国际事件。
...
- 要有预测验证和反证：拿昨日精选/昨日判断验证今天，再写明什么现象出现会推翻自己的主判断，体现"如果A发生我做B，如果C发生我做D"。
- 板块要涨跌两半对照：既写领涨主线，也写下跌方向和拖累项；涨的写"为什么涨/谁最核心"，跌的写"风险在哪里/是否扩散"。
- 个人操作段要有仓位纪律与心态：高抛、低吸、回血、留子弹，不自夸神操作，把仓位管理写成应对市场不确定性的工具。
- 句子要有人的温度：允许"当然""好了""若守不住，那就别犹豫"这类口语化转场，能对读者喊话给条件式建议。
- 结尾要有一点哲理，但必须从当天盘面推出来，别空喊鸡汤；像"高抛不是看空，而是为更好的低吸留出空间"这种，有交易纪律支撑才写。"""
...
- 缠论看结构：顶/底分型用于观察短线转折，箱体/中枢雏形用于判断震荡区间，缺口用于观察突破或回踩确认，背离/顶背离用于提示冲高回落风险；但没有分时数据时，只能基于日线和已给点位做级别较大的推演，禁止硬写5分钟MACD金叉或精确买卖点。
...
- 基本面看快讯：只从【当日重要资讯】解释题材背景。先判断快讯是否与今日热点/连板股相关，再写资金为什么切到该方向；不相关就不写消息面，不能为了完整而编新闻。
- 指数周期和短线情绪分开：指数结构可以修复但短线情绪走弱，也可以指数震荡但题材情绪强。不要因为下跌家数多就直接否定指数趋势，也不要因为指数收红就忽视个股亏钱效应。
- 板块看资金：涨幅靠前/跌幅靠前对比→资金流向；热点内龙头强、跟风弱=资金聚焦核心标的、结构分化，选股侧重点与节奏比方向更重要；跟风股补跌=退潮信号。
- 情绪判定要有标准："分歧加大"不等于"退潮确认"。退潮至少要看炸板率、连板晋级、昨日涨停反馈、高位断板反馈、跌停/大面、龙头负反馈、首板到二板成功率等多项共振；单一信号只能写需警惕，不要直接下重结论。
- 少说主力出货：没有成交量、换手、分时承接、龙虎榜、筹码结构等多证据时，不写"主力拉高出货"，改写为"高位筹码兑现压力增加，需观察放量后的承接质量"。
- 结论要多重信号印证：情绪+量能+技术结构+快讯背景共振才下结论，单一信号不冒险。
- 操作给条件预案：关键位+双分支（守住/失守）→具体动作，把风险和操作绑在一起说，仓位管理作"应对之锚"。
- 每个主判断都要留反证：如果最高板继续晋级、炸板率回落、昨日涨停溢价修复，就可能不是退潮而只是强分歧；如果指数关键位放量收复，就可能不是破位而是回踩确认。"""
...
def _build_prompt(report: dict, index_tech: dict, personal_note: str,
    ...
    if kx:
        news_inst = ('。基本面视角只按下方【当日重要资讯】来写：优先挑与今日热点板块/连板股相关的异动'
                     '（如"算力租赁概念异动、城地香江3连板"这类盘面异动），按"快讯触发→板块异动→龙头/跟风结构→持续性判断"补一句"消息面上，……"；'
                     '只有全部都不相关才不写。只写下方真实出现的，禁止编造新闻、政策、涨价、业绩或国际事件')
    else:
        news_inst = '。没有相关资讯就只写盘面资金流和技术结构，不要编造消息面或基本面原因'
...
【第3段 热点板块】开头写"从板块来看，"，选 2-3 个行业热点 + 1-2 个细分概念题材，每个写"XX走强/活跃，龙头A四连板，B、C等跟涨"（范文式）。板块段要写出"气势"：用"持续走强/震荡反弹/双双涨停/领涨"这类动感词，别把板块名和涨停数逐条罗列成数据清单。封板率、炸板数、成交额这类全市场指标不要写进板块段（它们属于第7段），涨停数只写本板块自己的。细分概念要写成精确概念名，只写下方"细分概念数据"里出现的，禁止编造概念名或个股归属。若有快讯支撑，要从基本面视角解释"为什么今天资金切到这里"，但只能引用下方真实快讯；没有相关快讯时，只写盘面资金流，不要编政策、业绩、涨价、国际事件{news_inst}。
行业热点数据：
{sectors_block}
细分概念数据：
{concept_block}
...
【第5段 观察盘面】开头写"观察盘面，认为："，用情绪周期信息写 2-3 句量能与轮动的点评。必须把指数周期和短线情绪拆开：指数结构修复不代表个股赚钱效应一定好，个股下跌多也不等于指数趋势已经坏。涨停时间分布（下方补充素材）可作为情绪佐证：早盘封板多=抢筹意愿强，午后封板多=分歧加大；有跌停家数时用它佐证恐慌是否可控。注意区分"分歧加大"和"退潮确认"，只有多项信号共振时才下退潮结论，否则写成"需警惕"或"强分歧"。
该段数据：情绪周期{cyc.get('stage')}；{cyc.get('stage_desc')}；建议{cyc.get('action')}。

【第6段 指数层面】开头写"指数层面："，用下方"点位数据"里真实的技术结构写 2-3 句。必须按缠论视角组织：先看指数处在什么结构位置（分型、箱体/中枢雏形、缺口、20日区间、均线），再看量能是否配合，再给当下级别的条件判断，最后落"守住关键位会怎样、失守该怎么应对"。可写阳包阴反包=转强、推动浪后的回撤属正常、顶背离=冲高回落预警、缺口未回补=做多意愿+回踩确认焦点、年线/60日均线=中期方向分水岭、整数关口=心理位。参考范文"双创指数与深成指……缺口有望成为突破性缺口"那种推演口吻，不要只贴点位就完事。数据里没有的信号（如5分钟MACD金叉、精确二买三买、某指数今日平开却写高开缺口）不要硬编。
点位数据：{idx_text}
...
【第8段 操作策略】开头写"操作策略："，写 4-5 句：①先验证昨日判断或昨日精选结果，用下方【昨日精选验证】真实结果说明哪些符合、哪些不符合②再给次日整体判断（竞价偏强/偏弱）③再给关注方向（哪些板块龙头或个股），若数据支持先点一句"核心矛盾或结构分化"（如热度集中在核心标的、跟风弱、指数与个股分化）再落到选股侧重④给确认条件（竞价高开幅度、封单、弱转强确认、量能和关键位）⑤明确不做什么（如龙头断板负反馈扩大时不接中位跟风、缩量反抽不追高）⑥次日关注方向可参考下方【补充素材】的龙虎榜净买与涨停复制候选（只写素材里真实出现的）⑦把仓位管理作"应对之锚"，收一句"宁可少赚，不可深套，不确定时及时离场"式纪律。
...
【第9段 结尾】以"值得强调的是，"开头，写 2-3 句心态劝勉 + 一句次日观察点。必须带一个反证条件：如果最高板继续晋级、炸板率回落、昨日涨停溢价修复，主判断可能要从退潮修正为强分歧；如果指数关键位放量收复，主判断可能要从破位修正为回踩确认。要有真实观点和温度，可用口语化转场（参考范文"当然，……也有""好了，……"的松弛感），也可把操作纪律升级成哲理性收束（如"高抛不是看空，而是为更好的低吸留出空间"），别写成干巴巴的总结。注意：结尾的哲理感悟段与"承蒙各位支持…"互动收尾会自动追加在文末，你只需写到这里为止，不要自己写这两段。"""
